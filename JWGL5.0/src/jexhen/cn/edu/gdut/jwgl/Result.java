...
public class Result {
	...
	private List<Score> allScoreResult;
	private List<Score> checkedScoreList = new ArrayList<Score>();
	...
	protected void createContents() {
		...
		qureyBySemester.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				allScoreResult = jwgl.getScores(yearSemesterCode);
				showResult(allScoreResult);//根据学年学期代码查询结果，并显示结果到界面
			}
		});
		...
		queryByYear.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				allScoreResult = jwgl.getScores(schoolYearNum);
				showResult(allScoreResult);//根据学年查询成绩，并且成绩显示到界面
			}
		});
		...
		queryAll.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				allScoreResult = jwgl.getAllScores(endYear);
				showResult(allScoreResult);
			}
		});
		...
		//显示查询结果的表格
		table = new Table(resultShell, SWT.BORDER | SWT.CHECK | SWT.FULL_SELECTION | SWT.MULTI);
	 ...
	 * 重新计算选择的课程的平均绩点
	 */
	private void recountGPA() {
		checkedScoreList.clear();//每次计算绩点之前要将需要计算绩点的课程集合清空
		for (int i = 0; i < table.getItemCount(); i++) {
			TableItem ti = table.getItem(i);
			if (ti.getChecked()) {
				Score score = findScoreByCourseName(ti.getText());
				if (score != null) {
					checkedScoreList.add(score);
				}
			}
		}
		if (!checkedScoreList.isEmpty() && checkedScoreList != null) {
			GPABar.setText("平均绩点: " + jwgl.countGPA(checkedScoreList));
		}
	}

	/**
	 * 根据课程名称查找Score实体
	 * @param coureName 课程名称
	 * @return 返回一个课程实体
	 */
	private Score findScoreByCourseName(String coureName) {
		if (!allScoreResult.isEmpty() && allScoreResult!=null){
			for (int i = 0; i < allScoreResult.size(); i++) {
				Score score = allScoreResult.get(i);
				if (score.getCourseName().equals(coureName)) {
					return score;
				}
			}
		}
