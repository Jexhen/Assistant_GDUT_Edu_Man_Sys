...
public class JWGL {
	...
	private CloseableHttpClient client;//HttpClient
	private String stuName;
	
	public JWGL(String usrname, String password) {
		this.usrname = usrname;
		this.password = password;
		this.cookie = getCookie();//根据用户名和密码获取cookie
	}
	...
	private List<Score> queryResult(String yearSemesterCode) {
		...
		try {
			...
			//抓取课程名称
			Pattern p = Pattern.compile("\"kcmc\":\"[^\\x00-\\xff[a-zA-Z][0-9]]+");
...
/*	public static void main(String[] args) {
		JWGL jwgl = new JWGL("3114004049", "Bys-42f,");
		if (jwgl.login() == 1) {
			List<Course> courses = jwgl.queryCourseChart("201502", 2);
